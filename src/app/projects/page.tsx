    ...
    <section className="mx-auto w-full max-w-[72rem] px-5 py-16 sm:px-8 sm:py-24 lg:px-12">
      ...
      <div className="mt-12 grid gap-10">
        {proofThemes.map((theme) => (
          <article key={theme.title} className="grid gap-4 border-t border-[color:var(--line)] pt-6">
            ...
            <p className="max-w-[48rem] text-base leading-8 text-[color:var(--muted)]">
              {theme.body}
      ...
      <div className="mt-16 grid gap-10 lg:grid-cols-2 lg:gap-16">
        <section className="border-t border-[color:var(--line)] pt-6">
          ...
          <ul className="mt-5 grid gap-3 text-base leading-8 text-[color:var(--muted)]">
            {openSource.map((item) => (
              <li key={item}>{item}</li>
            ))}
          </ul>
        ...
        <section className="border-t border-[color:var(--line)] pt-6">
          ...
          <ul className="mt-5 grid gap-3 text-base leading-8 text-[color:var(--muted)] sm:grid-cols-2">
            {skills.map((item) => (
              <li key={item}>{item}</li>
            ))}
          </ul>
